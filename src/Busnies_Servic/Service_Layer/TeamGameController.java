...
public class TeamGameController{
    ...
    public String create_team(String arg_name, String arg_field){
        ...
        if((LogicManagement.getCurrent().getPermissions().check_permissions((Action.Edit_team)) == 0) && !(LogicManagement.getCurrent() instanceof  TeamOwner)){
            return "You are not allowed to perform actions on the group.";
        }
        Team team = LogicManagement.findTeam(arg_name);
        if(team != null){
            return "The Team already exists in the system.";
        }
        Team new_team = new Team(arg_name,arg_field);
        LogicManagement.addToListTeam((new_team));
        new_team.set_TeamOwner((TeamOwner)LogicManagement.getCurrent());
    ...
    public String add_or_remove_player(String name_team, String user_name, int add_or_remove ){
        ...
        Subscription player = LogicManagement.contain_subscription(user_name);
        if(!(player instanceof Player)){
            return "The username is not defined as a player on the system.";
        }
        Team team = LogicManagement.findTeam(name_team);
        if(team != null){
    ...
    public String add_or_remove_coach(String name_team, String coach_add, int add_or_remove){
        ...
        Subscription coach = LogicManagement.contain_subscription(coach_add);
    ...
    public String add_or_remove_TeamOwner(String name_team, String TeamOwner, int add_or_remove){
        ...
        Subscription teamOwner = LogicManagement.contain_subscription(TeamOwner);
        ...
        if(add_or_remove == 1){
          ...
          ((TeamOwner) teamOwner).setAppointed_by_teamOwner(LogicManagement.getCurrent());
            return team.Edit_TeamOwner((TeamOwner) teamOwner,add_or_remove);
        }
        // remove teamOwner to team
        if(add_or_remove == 0){
            Subscription appointed = ((TeamOwner) teamOwner).getAppointed_by_teamOwner();
            if(LogicManagement.contain_subscription(appointed.getUserName()) != null ){
                // The person responsible for appointing the team is still in the system
                if(appointed != LogicManagement.getCurrent()){
    ...
    public String add_or_remove_TeamManager(String name_team, String TeamOwner, int add_or_remove){
        ...
        Subscription teamManager = LogicManagement.contain_subscription(TeamOwner);
        ...
        if(add_or_remove == 1){
            ...
            ((TeamManager) teamManager).setAppointed_by_teamOwner(LogicManagement.getCurrent());
    ...
    private String check_input_edit_team(String name_team, String user_name){
        if((LogicManagement.getCurrent().getPermissions().check_permissions((Action.Edit_team)) == 0)){
            return "You are not allowed to perform actions on the group.";
        }
        Team team = LogicManagement.findTeam(name_team);
        if(team == null){
        ...
        if(!team.check_if_object_in_team(LogicManagement.getCurrent()) || (LogicManagement.getCurrent() instanceof SystemAdministrator)){
            return "You are not allowed to perform actions in this group.";
        }
        Subscription subscription = LogicManagement.contain_subscription(user_name);
    ...
    public String change_status_team(String name_team, int status){
        ...
        String ans = check_input_edit_team(name_team,LogicManagement.getCurrent().getUserName());
        if(ans != null){
            return ans;
        }
        Team team = LogicManagement.findTeam(name_team);
        ...
        if(LogicManagement.getCurrent().getPermissions().check_permissions(Action.Close_team) == 0 ){
            return "You are not allowed to close a team.";
        }

        if(!(LogicManagement.getCurrent() instanceof SystemAdministrator)){
            //Only an administrator can permanently close the team
            if(status == -1){
                return "You are not authorized to perform this action.";
            }
        }else{
            if(LogicManagement.getCurrent().getPermissions().check_permissions(Action.Close_team_perpetually) == 0 ){
                return "You are not allowed to close a team.";
            }
        }
